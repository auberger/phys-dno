...
def sparce_coo_matrix2tensor(arr_coo, make_dense=False):
    # Check if it's a COO matrix in a way that works with both old and new scipy versions
    assert hasattr(arr_coo, 'format') and arr_coo.format == 'coo', f"arr_coo should be a coo_matrix, got {type(arr_coo)}. Please download the updated SKEL pkl files from https://skel.is.tue.mpg.de/."
