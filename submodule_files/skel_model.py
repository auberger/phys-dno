...
class CustomUnpickler(pkl.Unpickler):
    def find_class(self, module, name):
        if module == 'scipy.sparse._coo':
            # Redirect to the current scipy's sparse module
...
class SKEL(nn.Module):
    ...
    def __init__(self, gender, model_path=None, custom_joint_reg_path=None, **kwargs):
        ...
        # Use custom unpickler to handle scipy version mismatch
        with open(skel_file, 'rb') as f:
            skel_data = CustomUnpickler(f).load()
        ...
        if isinstance(skel_data['J_regressor'], scipy.sparse.spmatrix):
        ...
        if custom_joint_reg_path is not None:
            ...
            if isinstance(skel_data['J_regressor_osim'], scipy.sparse.spmatrix):
        ...
        if isinstance(skel_data['skin_weights'], scipy.sparse.spmatrix):
            self.register_buffer('skin_weights', sparce_coo_matrix2tensor(skel_data['skin_weights']))
        else:
            self.register_buffer('skin_weights', torch.FloatTensor(skel_data['skin_weights']))

        # Skeleton model skinning weights
        if isinstance(skel_data['skel_weights'], scipy.sparse.spmatrix):
        ...
        if isinstance(skel_data['skel_weights_rigid'], scipy.sparse.spmatrix):
