...
class AnatomicalJointFitter:
    ...
    def _optimize_batch(self, batch_params, max_iterations, learning_rate, pose_regularization):
        """
        Optimize a batch of frames using LBFGS optimizer.
        ...
        optimizer = torch.optim.LBFGS(
            [batch_params['poses'], batch_params['trans']],
            lr=learning_rate,
        ...
        for iter_idx in range(max_iterations):
            ...
            # Early stopping if the error is very low
            ...
            if self.debug:
                ...
                print(f"  Iteration {iter_idx}: Joint error: {error_mm:.2f} mm")
    ...
    def _compute_losses(self, predicted_joints, target_joints, poses, pose_regularization):
        ...
        # Anatomical constraints (enforce joint angle limits)
        anatomical_loss = torch.tensor(0.0, device=self.device)
